 ...
 *    durable main log.
 *
 * All data crosses the `btwPanel` Typert Remote namespace (status / followup /
 * cancel). Styles are injected as a scoped `<style data-dsh-btw>` element and
 * removed with the plugin's effect lifetime.
 *
...
export function apply(ctx) {
  ...
  // Command card: a resumable side thread. Polls the btwPanel remote; pauses
  ...
  function BtwCard(props) {
    ...
    const remote = ctx.remote && ctx.remote.btwPanel;

    React.useEffect(() => {
      let disposed = false;
      const poll = () => {
        if (!remote) return;
        remote.status(node.commandId).then((value) => {
          if (disposed) return;
          setChecked(true);
    ...
    const submitFollowup = () => {
      ...
      remote.followup(node.commandId, text, signal).then(() => {
    ...
    const cancelAsk = () => {
      if (!remote) return;
      remote.cancel(node.commandId, new AbortController().signal).then(() => {}, () => {});
    };
